...
void handle_add(char *data, char *response) {
    ...
    if (!title || !authors || !year || !path ||
        strlen(year) != 4 || strspn(year, "0123456789") != 4) {
        snprintf(response, BUFFER_SIZE, "Erro: dados inválidos (verifica se o ano tem 4 dígitos numéricos).");
        return;
    }
    
    if (strlen(title) == 0 || strlen(authors) == 0 || strlen(path) == 0) {
        snprintf(response, BUFFER_SIZE, "Erro: título, autores ou caminho não podem estar vazios.");
        return;
    }
